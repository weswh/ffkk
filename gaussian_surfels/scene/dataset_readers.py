...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        width = intr.width        
        #os.path.join(
        image_path = os.path.join(images_folder, 'images',os.path.basename(extr.name))
        #print( '$$$$$$$$$$$$',image_path )
        ...
        if intr.model=="SIMPLE_PINHOLE":
            ...
            # 使用 OpenCV 模型参数进行畸变校正
           
            fx = intr.params[0]  # 焦距 x
            fy = intr.params[1]  # 焦距 y
            cx = intr.params[2]  # 主点 x
            cy = intr.params[3]  # 主点 y
            k1 = intr.params[4]  # 畸变系数 k1
            k2 = intr.params[5]  # 畸变系数 k2
        
            # 构建相机矩阵 K 和畸变系数 D
            K = np.array([[fx, 0, cx], [0, fy, cy], [0, 0, 1]])
            D = np.array([k1, k2, 0, 0])  # 使用 k1 和 k2 畸变参数，其余畸变参数设置为0
        ...
        try:
            monoN = read_monoData(f'{images_folder}/../DSINE/{image_name}_normal.npy')
            try:
        ...
        #print(f'{images_folder}/u_masks/feet/select/{image_name[-4:]}.png')
        try:
            #mask = load_mask(f'{images_folder}/../mask/{image_name[-3:]}.png')[None]
            mask = load_mask(f'{images_folder}/masks/feet/select/{image_name[-4:]}.png')[None]
...
def fetchPly(path):
    ...
    # return  BasicPointCloud(points=downsampled_positions, colors=downsampled_colors, normals=downsampled_normals)

    return BasicPointCloud(points=positions, colors=colors, normals=normals)

def fetchPly2(path,cam):
    plydata = PlyData.read(path)
    vertices = plydata['vertex']
    ...
    K=np.array([[cam.width / (2 * math.tan(cam.FovX / 2.)),0,cam.prcppoint[0]*cam.width],
                [0,cam.height / (2 * math.tan(cam.FovY / 2.)),cam.prcppoint[1]*cam.height],
                [0,0,1]
    ])
    R=cam.R
    t=cam.T
    image=cv2.imread(cam.image_path)
    #print(image)
    ...
    num_iterations = 1 # Number of times to execute the process
    ...
    for _ in range(num_iterations):
        for face in faces:
            ...
            points = np.array(positions)  # shape: (N, 3)
            colors = np.array(colors)  # shape: (N, 3)
            normals = np.array(normals)  # shape: (N, 3)

            # Transform all points to camera coordinates in one go
            points_cam = np.dot(points, R.T) + t.reshape((1, 3))  # points_cam shape: (N, 3)

            # Project all points onto camera plane
            pixel_coords = np.dot(points_cam, K.T)  # pixel_coords shape: (N, 3)
            pixel_coords[:, :2] /= pixel_coords[:, 2][:, np.newaxis]  # Normalize by depth component

            # Determine which points fall within any face bounding box
            ...
            # Filter points, colors, and normals based on the mask
            points_in_face = points[mask]
            colors_in_face = colors[mask]
            normals_in_face = normals[mask]
            #pcd2=BasicPointCloud(points=points_in_face,colors=colors_in_face,normals=normals_in_face)
            # if len(points_in_face) > 0:
            #     dense_points = dense_point_cloud(points_in_face, colors_in_face, normals_in_face)
            #     pcd=combine_point_clouds(pcd,dense_points)

    numbers = [x for x in range(len(positions))]
    desired_samples = 350000
    stride = math.floor(len(numbers) / desired_samples)
    # 确保最后一个索引不超出范围
 
    #sample_indices = random.sample(range(len(numbers)), 400000)
    sample_indices = [i * stride for i in range(desired_samples)]
    if sample_indices[-1] >= len(numbers):
        sample_indices[-1] = len(numbers) - 1
    #uniform_sample = [numbers[idx] for idx in uniform_indices]
    positions=[positions[i] for i in sample_indices]
    colors=[colors[i] for i in sample_indices] 
    normals=[normals[i] for i in sample_indices]
    positions=np.concatenate((positions, points_in_face), axis=0)
    colors=np.concatenate((colors, colors_in_face), axis=0)
    normals=np.concatenate((normals, normals_in_face), axis=0)
    return BasicPointCloud(points=positions, colors=colors, normals=normals)
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    reading_dir = "u_images" #if images == None else images
...
def dense_point_cloud(points,colors,normals):
    nn = NearestNeighbors(n_neighbors=10, algorithm='kd_tree').fit(points)
    dense_points_indices = nn.kneighbors(return_distance=False)
    dense_points = points[dense_points_indices.reshape(-1)]
    dense_colors = colors[dense_points_indices.reshape(-1)]
    dense_normals = normals[dense_points_indices.reshape(-1)]

    return BasicPointCloud(points=dense_points, colors=dense_colors, normals=dense_normals)
    ...
    try:
        with open(os.path.join(path, transformsfile)) as json_file:
            ...
            for idx, frame in enumerate(frames):
                #print(os.path.join(path, frame["file_path"]))
                ...
                #图像及掩模数据
                pat=frame['file_path']
                image_path = f'{path}/u_images/'+frame["file_path"].split('/')[-1]#+extension
                #image_path = f'{path}/'+frame["file_path"].split('/')[1]+'/'+frame["file_path"].split('/')[2]#+extension
                ...
                mask = load_mask(f'{path}/u_masks/select/'+frame["file_path"].split('/')[-1])#+extension)
                arr = norm_data[:,:,:3] #* mask + bg * (1 - mask)
                image = Image.fromarray(np.array(arr*255.0, dtype=np.byte), "RGB")
                mask= torch.from_numpy(mask)
                mask=mask.unsqueeze(2).cpu().numpy()
                #print('mask:',np.array(mask).shape)
                mask = mask.transpose([2, 0, 1]).astype(np.float32)


                #相机内参
                fovx=2*math.atan(frame['w']/(2*frame['fl_x']))
                cx=frame['cx']
                cy=frame['cy']
                ...
                prcppoint = np.array([cx /frame['w'], cy /frame['h']])
                # prcppoint = np.array([cx /  contents['w'], cy /  contents['h']])
                prefix = transformsfile.split('.')[0]#.split('_')[1]
                #print(f'{path}/metric3D/{image_name}_normal.npy')

                #读取法线及深度数据
                try:
                    monoN = read_monoData(f'{path}/DSINE/{image_name[-3:]}_normal.npy')#.transpose(2,0,1)#.reshape(1, 3, 1080, 1920)
                    # print(f'{path}/stable/{image_name[-3:]}.jpg')
                    # monoN = cv2.imread(f'{path}/stable/{image_name[-3:]}.jpg')
                    # monoN_rgb = cv2.cvtColor(monoN, cv2.COLOR_BGR2RGB)
                    # monoN = np.array(monoN_rgb)
                    # monoN =(monoN /255.0)*2-1
                    # monoN=monoN.transpose(2,0,1)
                    #print(monoN)
                    try:
                        monoD = read_monoData(f'{path}/depth_npy/{image_name}_depth.npy')
                        # monoD=monoD/255.0
                        #print(f'{path}/normal/{image_name}_depth.npy')
                    except FileNotFoundError:
                        monoD = np.zeros_like(monoN[:1])
                    mono = np.concatenate([monoN, monoD], 0)
                except FileNotFoundError:
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    ply_path = os.path.join(path, "./outputs/points3D.ply")
    #cam=train_cam_infos[20]

    if os.path.exists(ply_path):
        #pcd = fetchPly2(ply_path,cam)
        pcd = fetchPly(ply_path)
        print(f"Featching points3d.ply...")
        # positions=np.concatenate((pcd.points, pcd.points), axis=0)
        # colors=np.concatenate((pcd.colors,pcd.colors), axis=0)
        # normals=np.concatenate((pcd.normals, pcd.normals), axis=0)
...
def combine_point_clouds(pc1: BasicPointCloud, pc2: BasicPointCloud) -> BasicPointCloud:
    combined_points = np.concatenate((pc1.points, pc2.points), axis=0)
    combined_colors = np.concatenate((pc1.colors, pc2.colors), axis=0)
    combined_normals = np.concatenate((pc1.normals, pc2.normals), axis=0)

    return BasicPointCloud(points=combined_points, colors=combined_colors, normals=combined_normals)
