...
def main(src, opts, exp_name=None, out_dir='eval_export/keypoints', gpu=0, no_rendering=True, produce_spins=False,
		export_meshes=True):
	no_rendering = True
	...
	dataset = Foot3DDataset(left_only=False, tpose_only=False, is_train=False, device=device)
	gt_loader = DataLoader(dataset, shuffle=False, collate_fn=collate_fn)

	template_foot = cfg['TEMPLATE_FEET'][0]
	template_dset = Foot3DDataset(left_only=False, tpose_only=True, specific_feet=[template_foot],
								  device=device)
	...
	for n, batch in enumerate(gt_loader):
		...
		# # 使用 trimesh 保存每一个 gt_mesh
		# tri_mesh = trimesh.Trimesh(vertices=verts, faces=faces)
		# tri_mesh.export(f"gt_mesh_{batch['name'][0]}.ply")
	...
	if export_meshes:
		...
		for n in range(len(predictions)):
			...
			for key, mesh in dict(gt_mesh=gt_mesh, pred_mesh=pred_mesh).items():
				...
				export_loc = os.path.join(mesh_dir, f'{n:02d}_{key}.ply')
				ply_data = trimesh.exchange.ply.export_ply(out_mesh)
				with open(export_loc, 'wb') as outfile:  # 'wb' for binary write
					outfile.write(ply_data)
